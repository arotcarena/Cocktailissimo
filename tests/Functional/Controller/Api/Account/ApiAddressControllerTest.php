...
class ApiAddressControllerTest extends FunctionalTest
{
    ...
    public function testNotLoggedUserCannotAccess()
    {
        ...
        //create
        $this->client->request('POST', $this->urlGenerator->generate('api_address_create', ['_locale' => 'en']), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseRedirects($this->urlGenerator->generate('security_login'));
        //update
        $this->client->request('POST', $this->urlGenerator->generate('api_address_update', ['id' => $id, '_locale' => 'en']), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseRedirects($this->urlGenerator->generate('security_login'));
        //delete
        $this->client->request('POST', $this->urlGenerator->generate('api_address_delete', ['_locale' => 'en']), [], [], [], json_encode($id));
        $this->assertResponseRedirects($this->urlGenerator->generate('security_login'));
    }
    ...
    public function testIndexReturnAddressesWithCorrectKeys()
    {
        ...
        $this->client->request('GET', $this->urlGenerator->generate('api_address_index', ['_locale' => 'en']));
        $this->assertEquals(
            ['id', 'civility', 'firstName', 'lastName', 'lineOne', 'lineTwo', 'postcode', 'city', 'country', 'state'],
    ...
    public function testIndexDatabaseQueries()
    {
        ...
        $this->assertLessThan(6, $dbCollector->getQueryCount());
    }
    ...
    public function testCreateWithInvalidDataContainingIncorrectKeys()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('api_address_create', ['_locale' => 'en']), [], [], [], json_encode([
    ...
    public function testUpdateWithInexistantIdParam()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('api_address_update', ['id' => '123456789456', '_locale' => 'en']), [], [], [], json_encode([]));
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertNotNull(
            json_decode($this->client->getResponse()->getContent())->errors
        );
    ...
    {
        $this->loginUser();
        $this->client->request('POST', $this->urlGenerator->generate('api_address_delete', ['_locale' => 'en']), [], [], [], json_encode(123456789456));
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertNotNull(
            json_decode($this->client->getResponse()->getContent())->errors
        );
    ...
    {
        return [
            'civility' => SiteConfig::CIVILITY_F,
            'firstName' => 'Justine',
            'lastName' => 'Trudo',
